...
SELECT s.store_id, SUM(p.amount) AS total_revenue FROM store s Inner JOIN staff st ON s.store_id = st.store_id
Inner JOIN payment p ON st.staff_id = p.staff_id GROUP BY s.store_id;

-- Question 7-Display the title of the movie, customer s first name, and last name who rented it.
SELECT f.title AS title, c.first_name, c.last_name FROM film f
...
SELECT a.first_name, a.last_name FROM actor a INNER JOIN film_actor fa ON a.actor_id = fa.actor_id
INNER JOIN film f ON fa.film_id = f.film_id WHERE f.title = 'Gone with the Wind';
-- Getting 0 records since there is no movie with that name
...
INNER JOIN film_category fc ON f.film_id = fc.film_id GROUP BY fc.category_id;
...
-- We cant use group by if we dont have aggregate function applicability
...
FROM customer c INNER JOIN rental r ON c.customer_id = r.customer_id INNER JOIN inventory i ON r.inventory_id = i.inventory_id
JOIN store s ON i.store_id = s.store_id WHERE s.store_id IN (1, 2) GROUP BY c.customer_id, c.first_name, c.last_name
